...
export function WhatsAppOrderButton({ product, size, colour, quantity, onValidation }: { product: Product; size?: string; colour?: string; quantity: number; onValidation: (errors: ProductSelectionErrors) => void }) {
  ...
  return <><button type="button" className="whatsapp-button" disabled={disabled} onClick={order}><MessageCircle aria-hidden="true"/> {disabled ? 'Currently out of stock' : 'Order on WhatsApp'}</button>{disabled && <p className="disabled-note">Ordering is unavailable because this item is out of stock.</p>}</>;
}
