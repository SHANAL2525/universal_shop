...
export default function CatalogueView() {
  ...
  return <>
    ...
    <section className="container catalogue"><div className="catalogue-tools"><SearchBar value={search.query} onChange={search.setQuery}/><CategoryFilter value={search.category} onChange={search.setCategory}/></div>
      {!search.query && search.category === 'All' && <section className="featured"><div className="section-heading"><div><span className="kicker">Editor’s selection</span><h2>Fresh arrivals</h2></div><span>{featured.length} curated pieces</span></div>{grid(featured)}</section>}
      <section><div className="section-heading"><div><span className="kicker">The collection</span><h2>{search.category === 'All' ? 'Shop all' : search.category}</h2></div><span>{search.filteredProducts.length} {search.filteredProducts.length === 1 ? 'piece' : 'pieces'}</span></div>{search.filteredProducts.length ? grid(search.filteredProducts) : <EmptyState query={search.query} onReset={search.reset}/>}</section>
    </section>
  </>;
}
