...
export function WhatsAppOrderButton({ product, size, colour, quantity, onValidation }: { product: Product; size?: string; colour?: string; quantity: number; onValidation: (errors: { size?: string; colour?: string }) => void }) {
  ...
  return <><button className="whatsapp-button" disabled={disabled} onClick={order}><MessageCircle/> {disabled ? 'Currently out of stock' : 'Order on WhatsApp'}</button>{disabled && <p className="disabled-note">Ordering is unavailable because this item is out of stock.</p>}</>;
}
